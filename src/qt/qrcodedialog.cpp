...
{
    QString ret = QString("growthcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non GRW unit input in lnReqAmount, we generate the URI with GRW as unit (as defined in BIP21)
